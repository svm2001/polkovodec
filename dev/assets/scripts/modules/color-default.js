export default function colorDefault() {
  ...
  colorItems.forEach(item => {
    ...
    item.addEventListener('click', () => {
      ...
      const customColorBlock = document.querySelector(
        '.colorsCustom__color-inner-wrapper',
      )
      const customColorSelectedValue = document.querySelector(
      ...
      if (customColorBlock)
        customColorBlock.style.backgroundColor = 'transparent'
      if (customColorSelectedValue)
        customColorSelectedValue.textContent = 'ral 0000'
    })
  })
}
