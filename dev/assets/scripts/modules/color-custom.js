...
export default function colorCustom() {
  ...
  const paintedBlock = colorsCustom.querySelector(
    '.colorsCustom__color-inner-wrapper',
  )
  const ralInput = colorsCustom.querySelector(
    '.colorsCustom__block-input-field',
  )
  const validationField = colorsCustom.querySelector(
    '.colorsCustom__block-input-validation',
  )
  const selectedColor = colorsCustom.querySelector(
    '.colorsCustom__color-selected-value',
  )

  const handleColorConfirm = () => {
    const inputValue = ralInput.value.trim()
    const foundColor = ralColors.find(
      color => color.ral === parseInt(inputValue),
    )
    ...
    if (foundColor) {
      ...
      // фиксированные цвета
      const fixedActiveColor = document.querySelector(
        '.colorDefault__item.active',
      )
